...
### Existing Supabase project: add Biology, Chemistry, and period-calendar fields

If you already ran an older version of this tracker, open **SQL Editor -> New query**
and run this once before deploying the Biology, Chemistry, and period-calendar updates:
...
create table if not exists period_cycles (
  ...
  constraint period_cycles_dates_valid check (end_date >= start_date)
);

alter table period_cycles enable row level security;

drop policy if exists "Allow all access to period cycles" on period_cycles;
create policy "Allow all access to period cycles"
  on period_cycles
  for all
  using (true)
  with check (true);
...
already creates these columns and the period-cycle table.
...
- Because there's no real Supabase Auth session, the database's Row Level
  ...
  more sensitive. This applies to the period-cycle table too, so avoid sharing
  the deployed URL and add real Supabase Auth before storing sensitive health data.
